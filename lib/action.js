...
function searchEvent(message, callback) {
  if ('text' in message.message) {
    ...
    event.eventSearchByName(searchText, (eventObj) => {
      if (eventObj) {
        msg.eventSearchResult(eventObj, () => {
          msg.send('REJOINDRE une publication ou effectuer une nouvelle recherche');
        });
      } else {
    ...
    msg.send("Quel est le nom de la publication à laquelle vous souhaitez participer ?");
  }
}
    ...
    event.eventSearchByName(searchText, (eventObj) => {
      if (eventObj) {
        msg.eventSearchPrintResult(senderId, eventObj, () => {
          msg.send('IMPRIMER une publication ou effectuer une nouvelle recherche');
        });
      } else {
        msg.send('Désolé, aucune publication trouvée...\nVeuillez faire une nouvelle recherche');
      }
...
function joinningEvent(message, callback) {
  ...
  EventData.findOne({ eventid: eventId }, (err, eventDataObject) => {
    ...
    if (eventDataObject) {
      ...
      if (bannedUser) {
        msg.send('Désolé, mais vous êtes banni(e) de ce groupe');

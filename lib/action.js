              ...
              UserData.update({ userid: senderId },
                { $push: { admin: eventObject.id } }, (err) => {
                  if (err) {
                    ...
                    msg.send('Votre publication est enregistrée', () => {
                      if (invitCode) {
      ...
      if ('hits' in results) {
        if (results.hits.total >= 1) {
          msg.eventSearchResult(results.hits.hits, () => {
            msg.send('REJOINDRE une publication ou effectuez une nouvelle recherche');
          });
        } else {
          msg.send('Désolé, aucune publication trouvé...\nVeuillez faire une nouvelle recherche');
        }
      } else {
        console.log('Error on serachEvent');
      }
    });
  } else {
    msg.send("Quel est le nom de la publication a laquelle vous souhaitez participer ?");
  }
}
...
function searchPrintEvent(message) {
  ...
  if ('text' in message.message) {
    ...
    (err, results) => {
      if ('hits' in results) {
        if (results.hits.total >= 1) {
          msg.eventSearchPrintResult(senderId, results.hits.hits, () => {
            msg.send('IMPRIMER une publication ou effectuez une nouvelle recherche');
          });
        } else {
          msg.send('Désolé, aucune publication trouvé...\nVeuillez faire une nouvelle recherche');
        }
    ...
    msg.send("Quel est le nom de la publication que vous souhaitez imprimer ?");
  }
}
...
function camera(message) {
  ...
  getContext(senderId, (context) => {
    if (context && 'joinEventId' in context) {
      eventData.findByEventID(context.joinEventId, (eventDataObjet) => {
        if (eventDataObjet) {
          if (eventDataObjet.moderators.indexOf(senderId) >= 0) { // TODO
            msg.photoMenuAdmin(senderId, context.joinEventId);
          } else {
            msg.photoMenu(senderId, context.joinEventId);
          }
        } else {
          console.log('Error in camera eventData.findByEventID');
        }
      });
    }
  });

    ...
    async.series({
      ...
      attachLocalUri: (callback) => {
        async.forEachSeries(printObj.photoList, (photo, callback) => {
          ...
          fs.stat(filePath, (err, stats) => {
            if (err) {
              const uri = cloudinary.url(`${photo.public_id}.jpg`, { effect: "viesus_correct", secure: true });
      ...
      coverLocalUri: (callback) => {
        ...
        fs.stat(filePath2, (err, stats) => {
          if (err) {
            const uri = cloudinary.url(`${printObj.cover_public_id}.jpg`, { effect: "viesus_correct", secure: true });

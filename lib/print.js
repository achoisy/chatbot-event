...
const print = module.exports = {
  ...
  updateOrder: (peechoReq, peechoUrl, callback) => {
    Print.findById(peechoReq.merchantId, (err, printObj) => {
      if (err) throw Error(`Error in print.updateOrder.Print.findById: ${err}`);

      if (printObj) {
        if (printObj.peechoOrderId === peechoReq.peechoId && printObj.ordered === true) {
          callback(printObj);
        } else {
          printObj.peechoOrderId = peechoReq.peechoId;
          printObj.ordered = true;
          printObj.peecho_url = peechoUrl;
          printObj.save(callback(printObj));
        }
      } else {
        throw Error('Empty printObj in print.updateOrder.Print.findById');
      }
    });
  },
  ...
  generatePdfPhantomjs: (printObj, callback) => {
    const printHtmlUrl = `${sslWebUrl}/print/pdf/${printObj._id}`;
        ...
        Print.findById(printObj._id, (err, updatePrintObj) => {
          ...
          if (updatePrintObj) {
            if (updatePrintObj.pdfUrl === `${sslWebUrl}/pdf/${printObj._id}.pdf`) {
              ...
              updatePrintObj.save(callback(updatePrintObj));
            }
          } else {
  ...
  sendPdfUrlToPeecho: (printObj, callback) => {
    ...
    peecho.generateSecret(printObj.peechoOrderId, printObj._id, printObj.pdfUrl,
      (genSecret) => {
        request.post(
          'http://test.www.peecho.com/rest/order/set_source_url',
          {
            form: {

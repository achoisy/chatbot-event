    ...
    async.series({
      eventDirectory: (callback) => {
        ...
        fs.stat(fullpath, (err, stats) => {
          ...
          if (err) {
            console.log(`imagestock/${printObj.eventId}/`);
            fs.mkdir(fullpath,
              callback(null, true));
          }
      ...
      attachLocalUri: (callback) => {
        async.forEachSeries(printObj.photoList, (photo, callback) => {
          ...
          fs.stat(filePath, (err, stats) => {
            if (err) {
              const uri = cloudinary.image(`${photo.public_id}.jpg`, { effect: "viesus_correct", secure: true });
      ...
      coverLocalUri: (callback) => {
        ...
        fs.stat(filePath2, (err, stats) => {
          if (err) {
            const uri = cloudinary.image(`${printObj.cover_public_id}.jpg`, { effect: "viesus_correct", secure: true });
            request(uri).pipe(fs.createWriteStream(filePath2))
            .on('close', callback(null, true));
          }
          if (!stats.isFile()) {

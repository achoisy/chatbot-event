...
const fonts = {
  Roboto: {
    ...
    bolditalics: '../fonts/roboto/Roboto-Italic.ttf',
  },
};
...
const print = module.exports = {
  ...
  updateOrder: (peecho, callback) => {
    Print.findOneAndUpdate(
      { _id: peecho.merchantId },
      { $set: { peechoOrderId: peecho.peechoId,
        ordered: true },
      }
    ).exec()
    .catch((err) => { throw Error(`Error in print.updateOrder.findOneAndUpdate: ${err}`); })
    .then(callback());
  },
  generatePdf: (printId, callback) => {
    print.findById(printId, (printObj) => {
      const docDefinition = {};

      docDefinition.content = [
        printObj.printTitle,
        {
          image: printObj.welcome_msg.photo,
        },
      ];

    ...
    User.findOne({ senderid: senderId }, (err, userObj) => {
      ...
      if (userObj) { // User exist
        if (!userObj.user_profile.profile_public_id) {
          cloudinary.uploader.upload(userObj.user_profile.profile_pic, { folder: "profile_pic", timeout: 60000 })
          .then((image) => {
        ...
        msg.getUser(senderId).then((user) => {
          ...
          newUser.save((err, updateUser) => {
            ...
            msg.send('Ceci est votre 1ère connexion ! Bienvenue.', () => {
              cloudinary.uploader.upload(user.profile_pic, { folder: "profile_pic", timeout: 60000 })
              .then((image) => {

            ...
            if (err) {
              ...
              newUserData.save((err) => {
                if (err) {
                  ...
                  msg.send('Ceci est votre 1ère connexion !', () => {
    ...
    User.findOne({ senderid: senderId }, (err, userObj) => {
      ...
      if (userObj) { // User exist
        if (!userObj.user_profile.profile_public_id) {
          cloudinary.uploader.upload(userObj.user_profile.profile_pic, (result) => {
            console.log("cloudinary profile_pic re-upload", result);
            if (result.public_id) {
              User.update({ _id: userObj._id }, { $set: { "user_profile.profile_public_id": result.public_id } })
              .exec();
            }
          });
        }
        ...
        msg.getUser(senderId).then((user) => {
          ...
          cloudinary.uploader.upload(user.profile_pic, (result) => {
            console.log("cloudinary profile_pic", result);
            const newUser = new User({
              senderid: senderId,
              user_profile: user,
              // next_payload: message.message.quick_reply.payload,
            ...
            newUser.save((err) => {
              ...
              msg.send('Ceci est votre 1ère connexion ! Bienvenue.', () => {
                callback(message);
              });
            });
          });

...
const messageFb = module.exports = {
  ...
  send: (msgText, senderId, callback) => {
  ...
  start: (senderId) => {
    const messageText = "Bienvenu sur le Chatbot de MonMagazine.fr";
    // const messageText = "Bienvenue";
    messenger.send({ text: messageText }, senderId)
    .then(() => {
      messenger.send(
      ...
      , senderId).then(() => {
        messenger.send({ text: 'Faites votre choix: CREER, PARTAGER ou IMPRIMER' }, senderId)
        .catch((err) => {
          console.error(`Erreur: ${err}`);
        });
      });
  ...
  confirmMobile: (mobileNumber, senderId) => {
  ...
  configEventMenu: (senderId) => {
    messageFb.send("Choix de l'image de couverture", senderId, () => {
      messenger.send(
        new fb.GenericTemplate([
          new fb.Element({
  ...
  recapEvent: (userObj, senderId) => {
    ...
    messenger.send({ text: recapEventText }, senderId).then(() => {
  ...
  welcomeToEvent: (eventObject, senderId, callback) => {
    let recapEventText = '';
    ...
    messenger.send({ text: recapEventText }, senderId).then(() => {
      ...
      , senderId).then(() => {
        if (eventObject.welcome_msg.audio !== '') {
          messenger.send(new fb.Audio({ url: eventObject.welcome_msg.audio }), senderId)
          .then(() => {
            callback();
          });
        } else {
  ...
  eventSearchResult: (hitList, senderId, callback) => {

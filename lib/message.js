...
const messageFb = module.exports = {
    ...
    messenger.send(
      new fb.GenericTemplate([
        new fb.Element({
          ...
          subtitle: 'Vous êtes invités à participer à une publication',
        }),
        new fb.Element({
        ...
        new fb.Element({
          title: 'Imprimer votre magazine',
          image_url: `${sslRootUrl}/images/square/print.png`,
          subtitle: 'Selectionnez vos publications et imprimez votre magazine.',
        }),
      ])
    ).then((res) => {
      const demqr1 = new fb.QuickReply({ title: `Creer`, payload: 'NEW_EVENT', image_url: `${sslRootUrl}/images/circle/add_event.png` });
      const demqr2 = new fb.QuickReply({ title: 'Participer', payload: 'PARTI_EVENEMENT', image_url: `${sslRootUrl}/images/circle/join_event.png` });
      const demqr3 = new fb.QuickReply({ title: `Imprimer`, payload: 'START_PRINT', image_url: `${sslRootUrl}/images/circle/print.png` });
      const demqrs = new fb.QuickReplies([demqr2, demqr1, demqr3]);
  ...
  choixPassword: () => {
    ...
    const addEventqr = new fb.QuickReplies([addEventqr1, addEventqr2]);
    messenger.send(Object.assign(
      { text: 'Voulez-vous mettre un code pour acceder a votre publication?' },
      addEventqr
    )).catch((err) => {
      ...
      messenger.send(
        new fb.GenericTemplate([
          new fb.Element({
            ...
            item_url: context.welcome_msg.photo.eager[0].secure_url,
    ...
    async.each(hitList, (hit, callback) => {
      hitListElements.push(
        new fb.Element({
          ...
          item_url: hit._source.welcome_msg.photo,

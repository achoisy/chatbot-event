...
const attach = module.exports = {
  ...
  addNew: (req, callback) => {
    ...
    // const transformation = JSON.parse(req.body.transformation);
  ...
  tranformeByPublicID: (publicId, callback) => {
    Attach.findOne({ public_id: publicId }, (err, attachObj) => {
      if (err) throw Error(`attach tranformeByPublicID.Attach.findOne: ${err}`);

      if (attachObj) {
        if (attachObj.transformation) {
          callback(attachObj.transformation);
        } else {
          callback(false);
        }
      } else {

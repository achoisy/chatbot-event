...
module.exports = (app) => {
  ...
  app.post('/messenger/cloture/close/:senderid/:eventid', (req, res) => {
    ...
    if (photoArray.length < 14) {
      res.sendStatus(400).send('Il faut un minimum de 14 photos pour cloturer la publication.');
    } else {
      print.cloture(senderId, eventId, req.body.photoArray, (printObj) => {
        if (printObj) {
          res.sendStatus(200).send("Felicitation, votre Publication est cloturée!");
        } else {
          res.sendStatus(400).send("Une erreur est survenu... la publication n'a pas été cloturée.");
        }
      });
    }
  });
  ...
  app.get('/messenger/full_image/:public_id', (req, res) => {
    // Proxied image
    const publicId = req.params.public_id;

...
//-------------------------------------------------------------------------------------------
const noCacheHeader = (req, res, next) => {
...
module.exports = (app) => {
  ...
  app.post('/messenger/uploads', (req, res) => {
    ...
    attach.addNew(req, (attachObject) => {
      if (attachObject) {
        ...
        res.render('message', { message: "Malheureusement, une erreur est servenue. Votre image n'est pas enregistrée. Veuillez réessayer ulterieurement ou contactez le service technique. Merci." });
      }
    });
  });
  ...
  app.route('/messenger/print/order')
    .get((req, res) => {
      peecho.checkSecret(req.query, (check) => {
        ...
        if (check) {
          print.updateOrder(req.query, (printObj) => {
  ...
  app.get('/messenger/profile_image/:public_id', (req, res) => {
    // Proxied image
    const publicId = req.params.public_id;
    const uri = cloudinary.url(`${publicId}.png`, { transformation: [
      { width: 200, height: 200, gravity: "face", radius: "max", crop: "crop" }, { width: 200 }] }
    );
    // console.log('Image proxie Filepath: ', uri);
    // req.pipe(request(uri)).pipe(res);
    request.get(uri).pipe(res);
